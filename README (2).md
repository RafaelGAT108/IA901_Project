...
Com a redefinição do pipeline (janelamento sem padding, extração das oito representações tempo-frequência em formato de array `.npz`, e treinamento via PyTorch Lightning com três arquiteturas distintas), os próximos passos do projeto são:

- Consolidar e comparar os resultados obtidos pelas três arquiteturas (ResNet50, InceptionV3, DenseNet121) sobre as oito representações individuais e as combinações multicanal, identificando qual combinação arquitetura/representação apresenta o melhor desempenho, sobretudo nas classes minoritárias.
- Avaliar o impacto do uso de pesos pré-treinados (ImageNet) em comparação ao treinamento do zero.
- Investigar estratégias adicionais para mitigar o desbalanceamento de classes além do sampler equalizador já implementado, já que a classe COPD permanece amplamente majoritária mesmo após o janelamento sem sobreposição.
- Atualizar o diagrama de workflow (`assets/workflow.png`) para refletir o pipeline atual.
- Comparar os resultados obtidos com outros algoritmos de aprendizado de máquina, além das CNNs já testadas.
